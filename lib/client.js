...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		var zh = {
			...
			"models.surfaces": "三端面（chat / responses / messages）均可用，tag 标注非 chat 模型。tag 为 unknown 的模型服务端没给出归类，可在这里手动指定端面、思考档位与多模态（留空即跟随默认）。",
			"models.surface": "端面",
			"models.follow": "跟随默认",
			"models.efforts": "思考档位",
		...
		var en = {
			...
			"models.surfaces": "All three surfaces (chat / responses / messages) work; tags mark non-chat models. A model tagged unknown was not classified by the server — set its surface, reasoning levels and modality by hand here (empty means follow the default).",
		...
		var S = {
			...
			select: { flex: "none", font: "inherit", fontSize: 12, padding: "3px 6px", borderRadius: 8, border: "1px solid var(--dsw-alias-border-l3, #444)", background: "var(--dsw-alias-bg-layer-1, #1e2126)", color: "var(--dsw-alias-label-primary, #e8eaed)" },
			ctl: { display: "inline-flex", alignItems: "center", gap: 4 },
			ctlLabel: { fontSize: 11, opacity: 0.6 },
