...
window.__ModuleLoader__.load({
	...
	factory: function (require) {
		...
		var zh = {
			...
			"models.surfaces": "三端面（chat / responses / messages）均可用，tag 标注非 chat 模型。tag 为 unknown 的模型服务端没给出归类，点每行「参数」可手动指定端面、思考档位与多模态（留空即跟随默认）。",
			"models.options": "参数",
			"models.surface": "端面",
			"models.follow": "跟随默认",
			"models.efforts": "思考档位",
		...
		var en = {
			...
			"models.surfaces": "All three surfaces (chat / responses / messages) work; tags mark non-chat models. A model tagged unknown was not classified by the server — open its \"options\" to set the surface, reasoning levels and modality by hand (empty means follow the default).",
