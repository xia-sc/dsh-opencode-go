...
import { IMAGE_MEDIA_TYPES, MAX_INLINE_IMAGE_BYTES, MODEL_TABLE, capabilitiesOf, endpointOf, reasoningFor } from "./models.js";
import {
...
export class OpencodeGoAdapter extends LlmAdapter {
  ...
  async listModels(provider) {
    ...
    const listed = MODEL_TABLE.filter((m) => enabled.includes(m.id)).map((m) => ({
      ...
      id: m.id,
      name: m.id,
      ...this.modelCapabilities(m.id),
    }));
    // Enabled ids the static table does not know (e.g. discovered via
    // refresh) stay selectable; requests route to the surface the user
    // declared for them (chat by default) and the server stays authoritative.
    for (const id of enabled) {
      if (!known.has(id) && !listed.some((m) => m.id === id)) {
        listed.push({ provider, id, name: id, ...this.modelCapabilities(id) });
      }
    }
    return listed;
  }

  /** The user's `modelCaps` entry for one id, or undefined when unset. */
  capOverride(model) {
    const caps = this.getOptions().modelCaps;
    if (!Array.isArray(caps)) return undefined;
    return caps.find((entry) => entry?.id === model);
  }

  /**
   * Declared capability fields for one id, shared by `listModels` and
   * `resolveModel` so the catalog and the request path cannot disagree.
   * `inputModalities` is omitted when the table cannot classify the id and the
   * user has not either: absent means "unknown" to the runtime, which stays
   * permissive. `reasoning` is omitted when there are no levels to offer.
   */
  modelCapabilities(model) {
    const { image, efforts } = capabilitiesOf(model, this.capOverride(model));
    const reasoning = reasoningFor(model, this.capOverride(model));
    return {
      ...(image === undefined ? {} : { inputModalities: image ? ["text", "image"] : ["text"] }),
      ...(reasoning === undefined ? {} : { reasoning }),
    };
  }

  /** Effective serving surface for one id (override, then table, then chat). */
  surfaceOf(model) {
    return capabilitiesOf(model, this.capOverride(model)).surface;
  }
  ...
  async resolveModel(provider, model) {
    ...
    // overrides (modelCaps) supply capacity, the surface, the modality and
    // the reasoning levels for ids the static table cannot classify.
  ...
  /**
   * This route's request image budget. `base64` because the wire carries inline
   * data URLs, so the accounting must match the encoded bytes the body will
   * actually hold, not the raw file sizes.
   */
  imageBudget() {
    return { representation: "base64", maxBytes: this.getOptions().maxRequestImageBytes };
  }

  /**
   * Replace every durably offloaded occurrence with the harness's placeholder
   * text. The offloaded set is a durable decision shared by every route, so a
   * route that re-inlines those bytes undoes the harness's own bookkeeping.
   * `projectOffloadedImages` always returns a fresh list.
   */
  projectOffloaded(messages) {
    return projectOffloadedImages(messages, (ref) => offloadedImageText(ref, this.services.imageAccess?.(ref)));
  }

  /**
   * Exact encoded byte length of every retained occurrence. A durable ref
   * carries its encoded size, so the normal path measures without touching the
   * disk; only a ref that lost that field is read to be measured, and that read
   * is reused when the bytes are finally inlined.
   */
  async measureImages(refs) {
    const sizes = new Map();
    const read = new Map();
    for (const ref of refs) {
      const id = String(ref.attachmentId);
      if (Number.isSafeInteger(ref.bytes) && ref.bytes > 0) {
        sizes.set(id, ref.bytes);
        continue;
      }
      const bytes = await this.readAttachmentFile(ref);
      read.set(id, bytes);
  ...
  /**
   * Refuse a request whose retained occurrences still exceed the route budget.
   *
   * The failure is the harness's own offload request, not a provider error: it
   * names how many of the oldest occurrences must go, and
   * `dsh-compaction-image-offload` records that selection and retries the step
   * (an agent turn or a compaction summary). Throwing instead of dropping
   * occurrences locally is deliberate — choosing which images a conversation
   * loses is a durable session decision, and only the harness can record it.
   */
  assertRequestImagesFit(messages, sizes) {
    const budget = this.imageBudget();
    const offloadImages = requiredImageOffload(messages, budget, (block) => {
      const id = String(block.attachment.attachmentId);
      const bytes = sizes.get(id);
      if (bytes === undefined) {
        throw new LlmError(
          `llm-opencode-go: request image ${id} was not measured`,
    ...
    if (offloadImages > 0) {
      throw new LlmError(
        `llm-opencode-go: request images exceed the ${budget.maxBytes}-byte inline budget; `
  ...
  /**
   * Read one occurrence's bytes through the attachment + fs services, applying
   * the per-image inline cap. Both injectable for tests.
   */
  async readAttachmentFile(ref) {
  ...
  /**
   * Resolve image refs to inline base64 payloads, reusing any bytes a caller
   * already read while measuring (see {@link measureImages}).
   */
  async resolveImageData(refs, read = new Map()) {
    const out = new Map();
    for (const ref of refs) {
      const id = String(ref.attachmentId);
      const bytes = read.get(id) ?? (await this.readAttachmentFile(ref));
      out.set(id, { mediaType: ref.mediaType, base64: Buffer.from(bytes).toString("base64") });
    }
    return out;
  }
  ...
  async *stream(options) {
    ...
    // Durable offload decisions come first: an occurrence the harness already
    // dropped must travel as placeholder text, never as bytes, and must not
    // count against the budget either.
    const request = { ...options, messages: this.projectOffloaded(options.messages) };
    const refs = collectImageRefs(request.messages);
    const { sizes, read } = await this.measureImages(refs);
    this.assertRequestImagesFit(request.messages, sizes);
    const images = refs.length > 0 ? await this.resolveImageData(refs, read) : new Map();
    // The user's per-model override can move an id to another surface, so
    // routing follows the same resolution as the declared capabilities.
    const cap = this.capOverride(options.model);
    const endpoint = this.surfaceOf(options.model);
    const meta = {
      t: Date.now(),
      model: options.model,
    ...
    if (endpoint === "responses") {
      yield* meteredStream(this.streamSurface(connection, headers, request, "/v1/responses", buildResponsesBody(request, images, cap), (body, signal, idleMs) => this.pumpResponses(body, signal, idleMs)), meta, record);
      ...
      yield* meteredStream(this.streamSurface(connection, msgHeaders, request, "/v1/messages", buildAnthropicBody(request, images, cap), (body, signal, idleMs) => this.pumpMessages(body, signal, idleMs)), meta, record);
    } else {
      yield* meteredStream(this.streamSurface(connection, headers, request, "/v1/chat/completions", buildChatBody(request, images, cap), (body, signal, idleMs) => this.pumpChunks(body, signal, idleMs)), meta, record);
    }
  }
...
/**
 * One settings-card row: the id plus the classification and modality this
 * route will actually use, so the card's tags and its reasoning presets agree
 * with the request path instead of guessing.
 *
 * `unknown` is a classification signal the card shows as a tag, not a routing
 * decision: it stays literal when neither the static table nor the user's
 * override says which surface serves this id, so the user can see that the id
 * is unclassified and fill it in below.
 */
function classifyModel(id, override) {
  const { image } = capabilitiesOf(id, override);
  return {
    id,
    surface: override?.surface ?? endpointOf(id) ?? "unknown",
    // Modality for the card tags; omitted when nothing declares it (the card
    // then shows the surface tag only).
    ...(image === undefined ? {} : { input: image ? ["text", "image"] : ["text"] }),
    // The levels the request path offers when the user states no override at
    // all. The card needs them to keep a hand-stated level list valid when the
    // user hands the choice back to the default: an unknown id follows the chat
    // surface yet deliberately offers no levels, so the honest answer is the
    // level list, not the surface.
    defaultEfforts: capabilitiesOf(id, undefined).efforts,
  };
}

/** The user's `modelCaps` entry for one id within an options snapshot. */
function overrideOf(caps, id) {
  return Array.isArray(caps) ? caps.find((c) => c?.id === id) : undefined;
}

/**
 * Static classification of the enabled ids — the same view `refreshModels`
 * reports, without touching the network. The settings card renders from this
 * before (or without) a live catalog refresh, so a non-chat model's reasoning
 * presets cannot offer a vocabulary its own surface would reject.
 */
export function knownModels(getOptions) {
  const options = getOptions();
  return ok({
    models: options.enabledModels.map((id) => classifyModel(id, overrideOf(options.modelCaps, id))),
  });
}
...
export async function refreshModels(getOptions, resolveKey, signal) {
  ...
  const models = [];
  const caps = connection.modelCaps;
  for (const entry of data) {
    if (entry !== null && typeof entry === "object" && typeof entry.id === "string" && entry.id !== "") {
      models.push(classifyModel(entry.id, overrideOf(caps, entry.id)));
    }
  }
  return ok({ models });
}
