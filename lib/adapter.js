...
export async function refreshModels(getOptions, resolveKey, signal) {
  ...
  const models = [];
  const caps = Array.isArray(connection.modelCaps) ? connection.modelCaps : [];
  for (const entry of data) {
    if (entry !== null && typeof entry === "object" && typeof entry.id === "string" && entry.id !== "") {
      const override = caps.find((c) => c?.id === entry.id);
      const cap = capabilitiesOf(entry.id, override);
      models.push({
        id: entry.id,
        // `unknown` is a classification signal the card shows as a tag, not a
        // routing decision: it stays literal when neither the static table nor
        // the user's override says which surface serves this id, so the user
        // can see that the id is unclassified and fill it in below.
        surface: override?.surface ?? endpointOf(entry.id) ?? "unknown",
        // Modality for the card tags; omitted when nothing declares it (the
        // card then shows the surface tag only).
        ...(cap.image === undefined ? {} : { input: cap.image ? ["text", "image"] : ["text"] }),
      });
    }
  }
  return ok({ models });
