...
import { MODEL_TABLE, SURFACES, SURFACE_EFFORT_IDS, endpointOf } from "./models.js";
import { OpencodeGoAdapter, knownModels, refreshModels } from "./adapter.js";
...
/**
 * Accumulated inline image bytes one request may carry, counted the way the
 * request body counts them (base64). Above this the adapter fails with
 * `IMAGE_OFFLOAD_REQUIRED` and lets the harness's compaction seam drop the
 * oldest occurrences and retry, instead of handing the endpoint a body it will
 * refuse. The operator publishes no request-size limit, so the default is a
 * deployment choice: 64 MiB inline is far above ordinary screenshot traffic and
 * still bounds a runaway history. Raise it (or lower it toward a proxy's body
 * cap) per deployment.
 */
export const DEFAULT_MAX_REQUEST_IMAGE_BYTES = 64 * 1024 * 1024;
...
export const Config = z.object({
  ...
  /**
   * Inline image budget for one request, in the bytes the body carries after
   * base64 expansion. Exceeding it is not an error the user sees: the adapter
   * reports `IMAGE_OFFLOAD_REQUIRED` with how many of the oldest occurrences to
   * drop, `dsh-compaction-image-offload` records the selection, and the request
  ...
  /**
   * Per-model overrides, user-filled (the operator discloses no context
   * metadata and no capability metadata for ids it has not published in
   * `MODEL_TABLE`). Entries carry only the fields the user set; an empty
   * array means "no overrides". Fields are optional by schemastery default
   * (only `id` is required).
   *
   * `contextWindow` / `maxTokens` state capacity; `surface` / `image` /
   * `efforts` state capability for an id the static table cannot classify —
   * e.g. a freshly served `deepseek-flash` whose catalog entry reports
   * `surface: "unknown"`, which would otherwise route to chat, declare no
   * modality, and offer no reasoning levels.
   */
  modelCaps: z.array(z.object({
    id: z.string().required(),
    contextWindow: z.number().step(1).min(1),
    maxTokens: z.number().step(1).min(1),
    surface: z.union([z.const("chat"), z.const("responses"), z.const("messages")]),
    image: z.boolean(),
    // `default(undefined)` on purpose: without it schemastery materializes `[]`
    // for an omitted array, which would silently mean "no reasoning levels" for
    // every entry that set some *other* field. Absent must stay absent so the
    // static table still governs, while an explicit `[]` means "no levels".
    efforts: z.array(z.string()).default(undefined),
  })).default([]),
});
...
export function resolveOptions(raw = {}) {
  ...
  const maxRequestImageBytes = raw.maxRequestImageBytes ?? DEFAULT_MAX_REQUEST_IMAGE_BYTES;
  if (!Number.isInteger(maxRequestImageBytes) || maxRequestImageBytes <= 0) {
    throw new Error("llm-opencode-go: maxRequestImageBytes must be a positive integer");
  }
  ...
  const caps = modelCaps.map((entry) => {
    ...
    if (entry.surface !== undefined) {
      if (!SURFACES.includes(entry.surface)) {
        throw new Error(
          `llm-opencode-go: modelCaps["${entry.id}"].surface must be one of ${SURFACES.join(" / ")}, got ${JSON.stringify(entry.surface)}`,
        );
      }
      out.surface = entry.surface;
    }
    if (entry.image !== undefined) {
      if (typeof entry.image !== "boolean") {
        throw new Error(`llm-opencode-go: modelCaps["${entry.id}"].image must be a boolean`);
      }
      out.image = entry.image;
    }
    if (entry.efforts !== undefined) {
      if (!Array.isArray(entry.efforts)) {
        throw new Error(`llm-opencode-go: modelCaps["${entry.id}"].efforts must be an array of reasoning level ids`);
      }
      // Levels are wire values, so they are checked against the surface that
      // will actually carry them — the override's own surface first, then the
      // static table, then the chat fallback the request path will use.
      const surface = out.surface ?? endpointOf(entry.id) ?? "chat";
      const allowed = SURFACE_EFFORT_IDS[surface] ?? [];
      for (const id of entry.efforts) {
        if (typeof id !== "string" || !allowed.includes(id)) {
          throw new Error(
            `llm-opencode-go: modelCaps["${entry.id}"].efforts entry ${JSON.stringify(id)} is not a ${surface}-surface reasoning level`
              + (allowed.length === 0 ? ` (the ${surface} surface has no level vocabulary)` : ` (allowed: ${allowed.join(", ")})`),
          );
        }
      }
      // Dedupe: the host rejects duplicate effort ids as invalid metadata.
      out.efforts = [...new Set(entry.efforts)];
    }
    return out;
  });
  return {
...
export function apply(ctx, config) {
  ...
  let lastRaw;
  let lastGood;
  // The last refused settings section, kept so the settings card can say why
  // its numbers and the request path disagree: a rejected section is dropped
  // whole, and without this diagnostic the only trace is a log line.
  ...
  const options = () => {
    ...
    try {
      ...
      lastRaw = raw;
      lastConfigError = error?.message ?? String(error);
  ...
  const adapter = new OpencodeGoAdapter(options, resolveKey, {
    ...
    // Placeholder text for a durably offloaded occurrence. The harness owns the
    // wording and the attachment-to-execution-world path bridge; this route
    // only supplies the two providers.
    imageAccess: (ref) => {
    ...
    const handle = async (envelope, signal) => {
      ...
      const args =
        ...
        rawPayload.args !== null && typeof rawPayload.args === "object"
          ? rawPayload.args
          : {};
      // Attach the current settings-section diagnostic to a successful catalog
      // reply. The card already polls this endpoint, so the refusal reaches the
      // user without a second round-trip; a refused section never changes the
      // reply's rows, but it explains why those rows are the composition's.
      const withConfigError = (envelope) =>
        envelope?.ok === true && envelope.value !== null && typeof envelope.value === "object"
          ? { ...envelope, value: { ...envelope.value, configError: lastConfigError } }
          : envelope;
      switch (envelope.method ?? "") {
        case "models/known":
          // Static classification for the card's rows: no key, no network, so
          // the surface/modality view always matches the request path.
          return withConfigError(knownModels(options));
        case "models/refresh":
          return withConfigError(await refreshModels(options, resolveKey, signal));
        case "usage/summary": {
