 ...
 * the happy path. The trailing `finally` covers the consumer `break` path:
 * leaving a `for await` via `return()` skips both the fallthrough and the
 * `catch`, so without it that row would vanish silently.
 */
export async function* meteredStream(source, meta, record) {
  ...
  try {
    ...
    // Consumer early-exit (`break` out of `for await`) bypasses both the
    // fallthrough and the catch: settle as aborted so the row is kept.
    done(finish?.kind ?? "aborted");
  }
}
